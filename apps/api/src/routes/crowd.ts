...
let inMemoryCameras: InMemoryCamera[] = [
  {
    id: "CAM-001",
    name: "Main Gate / Concourse",
    location: "North Gate Entrance",
    source: "rtsp://live.stream/gate1",
    sourceType: "rtsp",
    status: "ONLINE",
    lastActive: new Date(),
  },
  {
    ...
    location: "South Gate Concourse",
    source: "rtsp://live.stream/gate2",
    sourceType: "rtsp",
    status: "ACTIVE",
    lastActive: new Date(),
  },
...
router.get("/cameras", async (_req, res): Promise<void> => {
  ...
  const cameras = inMemoryCameras.map((camera) => ({ ...camera, lastActive: camera.lastActive.toISOString() }));
  res.json(GetCamerasResponse.parse(cameras));
});
...
router.post("/cameras", async (req, res): Promise<void> => {
  ...
  inMemoryCameras.push(camera);
  res.status(201).json(CreateCameraResponse.parse({ ...camera, lastActive: camera.lastActive.toISOString() }));
});
...
router.patch("/cameras/:cameraId", async (req, res): Promise<void> => {
  ...
  const idx = inMemoryCameras.findIndex((c) => c.id === params.data.cameraId);
  if (idx === -1) { res.status(404).json({ error: "Camera not found" }); return; }
  inMemoryCameras[idx] = { ...inMemoryCameras[idx], ...body.data };
  const updated = inMemoryCameras[idx];
  res.json(UpdateCameraResponse.parse({ ...updated, lastActive: updated.lastActive.toISOString() }));
});
...
router.delete("/cameras/:cameraId", async (req, res): Promise<void> => {
  ...
  inMemoryCameras = inMemoryCameras.filter((c) => c.id !== params.data.cameraId);
  res.sendStatus(204);
});
